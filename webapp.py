...
def render_fact():
    ...
    secondcounty = county_most_veterans(state)
    otherFact = "In " + state + ", the county with the highest veteran count is " + secondcounty + "."
    return render_template('home.html', state_options=states, funFact=fact, otherFact=otherFact)
...
def county_most_veterans(state):
    """Return the name of a county in the given state with the most veterans"""
    with open('demographics.json') as demographics_data:
        counties = json.load(demographics_data)
    highest=0
    county = ""
